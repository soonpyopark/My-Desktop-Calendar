# My Desktop Calendar v1.1.8

Windows 전용 데스크톱 캘린더입니다. **WPF (.NET 8) 네이티브 셸** + **React UI**(WebView2)로 동작합니다.

월·주·연 보기, 음력·음력 반복 일정, 대한민국 공휴일, JSON/ICS/CSV 가져오기·내보내기, **바탕화면 모드(창 잠금)**, **시스템 트레이**, **MSI 설치판**, **로컬 HTTP 웹 동기화**를 지원합니다.
...
| 구성 | 설명 |
|------|------|
| **셸** | **WPF (.NET 8) 단일 HWND** — MainWindow + WebView2 하나 |
...
| **바탕화면 모드** | 창 잠금(이동·크기조절·최소화·최대화·닫기 불가) + 다른 창 아래 유지 + Win+D에도 유지. 바탕화면 `SetParent` 임베드 없음 |
| **창 모드** | 같은 창을 잠금 해제 — 이동·리사이즈·타이틀바 컨트롤 |
...
- **바탕화면 모드** — 잠금 + 항상 다른 창 아래; Win+D로도 숨기지 않음; 날짜 더블클릭·퀵편집 시 일시적으로 맨 앞
- **최초 실행** — 주 모니터에 기본 위치·크기(5픽셀 단위); 이후에는 사용자가 저장한 위치·크기로 실행. 기동은 항상 바탕화면 모드
- **프레임리스 창** — 커스텀 타이틀바 + 가장자리 리사이즈; 리사이즈 중에는 고스트로 콘텐츠 재배치를 줄임
- **날짜 칸 더블클릭** — 퀵 편집(제목 `7. 9.(목) (음 5. 25.)`, 종일 추가, 완료 체크, 날짜색·연필→전체 편집기)
- **일정 바** — 좌클릭 퀵 편집; **우클릭 → 상세**; 완료 시 취소선(배경색 없음)
