...
월·주·연 보기, 음력·음력 반복 일정, 대한민국 공휴일, JSON/ICS/CSV 가져오기·내보내기, **바탕화면 위젯(이중 HWND 임베드)**, **시스템 트레이**, **MSI 설치판**, **로컬 HTTP 웹 동기화**를 지원합니다.
...
| 구성 | 설명 |
|------|------|
| **셸** | **WPF (.NET 8) 이중 HWND** — DesktopHost만 1회 `SetParent`, 모드 전환은 Show/Hide (무깜빡임) |
...
- **창 모드 / 바탕화면 모드** — 헤더·트레이에서 전환 (크기·위치 유지); App ↔ DesktopHost **월/주 위치 동기화**
...
- **로그인 유지** — `data/admin-sessions.json` (App·Host WebView 세션 동기화)
- **음력 반복** — 음력 매년·매월
- **테마** — 라이트 / 다크 / 시스템
